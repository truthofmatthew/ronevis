...
final class DisplayManagerCollector {
     ...
     */
    @NonNull
    private static String activeFlags(int bitfield) {
        final StringBuilder result = new StringBuilder();
        // Look for masks, apply it an retrieve the masked value
        for (int i = 0; i < DisplayManagerCollector.mFlagsNames.size(); i++) {
            final int maskValue = DisplayManagerCollector.mFlagsNames.keyAt(i);
            final int value = bitfield & maskValue;
            if (value > 0) {
                if (result.length() > 0) {
                    result.append('+');
                }
                result.append(DisplayManagerCollector.mFlagsNames.get(value));
            }
        }
        return result.toString();
    }
